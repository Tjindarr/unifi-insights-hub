...
import { ArrowDown, ArrowUp, Wifi, Cable, Gauge } from "lucide-react";

import { PageHeader, StatTile } from "@/components/app-shell";
import { overview, wanThroughput } from "@/lib/mock-data";
import { formatBits, formatBytes, formatRate, formatTime } from "@/lib/format";

export const Route = createFileRoute("/_authenticated/")({
  head: () => ({
    meta: [{ title: "Overview — UniFi Dashboard" }],
  }),
    ...
    <div>
      ...
      <div className="p-6 space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            sub={
              <span className="flex items-center gap-3">
                <span className="flex items-center gap-1">
                  <Wifi className="h-3 w-3" /> {o.wireless} wireless
                </span>
                <span className="flex items-center gap-1">
                  <Cable className="h-3 w-3" /> {o.wired} wired
                </span>
              </span>
            }
          />
          <StatTile
            label="WAN download"
            value={formatBits(o.currentRx)}
            sub={formatRate(o.currentRx)}
            accent="rx"
          />
          <StatTile
            label="WAN upload"
            value={formatBits(o.currentTx)}
            sub={formatRate(o.currentTx)}
            accent="tx"
          />
          <StatTile
            label="Avg satisfaction"
            value={`${o.avgSatisfaction}%`}
            sub={
              <span className="flex items-center gap-1">
                <Gauge className="h-3 w-3" /> across all clients
              </span>
            }
          />
        </div>
          ...
          <div className="flex items-center justify-between px-4 pt-4">
            ...
            <div className="flex items-center gap-4 text-xs">
              <span className="flex items-center gap-1.5">
                <span className="h-2 w-2 rounded-full bg-rx" /> RX
              </span>
              <span className="flex items-center gap-1.5">
                <span className="h-2 w-2 rounded-full bg-tx" /> TX
              </span>
            </div>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={wanThroughput} margin={{ top: 10, right: 12, left: 0, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v: number, name) => [formatBits(v), name === "rx" ? "RX" : "TX"]}
                />
                <Area
                  type="monotone"
                  dataKey="rx"
        ...
        <div className="rounded-lg border border-border bg-card">
          <div className="px-4 py-3 border-b border-border">
            ...
            <p className="text-xs text-muted-foreground">By current RX + TX</p>
          </div>
                ...
                <li key={c.id} className="px-4 py-2.5">
                  <div className="flex items-center justify-between gap-4 text-sm">
                    ...
                    <div className="flex items-center gap-4 text-xs tabular-nums whitespace-nowrap">
                      <span className="text-rx flex items-center gap-1">
                        <ArrowDown className="h-3 w-3" />
                        {formatBits(c.rxRate)}
                      </span>
                      <span className="text-tx flex items-center gap-1">
