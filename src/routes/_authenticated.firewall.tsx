...
function FirewallPage() {
  ...
  function applyCustom() {
    if (!customFrom || !customTo) {
      setRangeError("Pick both From and To.");
      return;
    }
    const f = new Date(customFrom).getTime();
    const t = new Date(customTo).getTime();
  ...
  const windowTotal = useMemo(
    () =>
      firewallByMinute.reduce(
        (sum, r) => sum + (Number(r.success) || 0) + (Number(r.failure) || 0),
        0,
      ),
    [firewallByMinute],
  );
        ...
        actions={
          <div className="flex items-center gap-2 flex-wrap justify-end max-w-[760px]">
            ...
            <button
              ...
              title={
                paused
                  ...
                  : "Pause auto-refresh while you search"
              }
            >
            ...
            <Input
              placeholder="Dest IP"
              value={dstQ}
              onChange={(e) => setDstQ(e.target.value)}
              className="h-8 w-32"
            />
            <Input
              placeholder="Port"
          ...
          <div className="ml-auto text-[11px] text-muted-foreground tabular-nums">
            {customActive ? (
              <>
                ...
                {" → "}
                {untilMs != null
                  ? new Date(untilMs).toLocaleString(undefined, { hour12: false })
                  : "now"}
                {" · "}
            ...
            {rows.map((e) => {
              ...
              return (
                <li
                  ...
                  className={cn(
                    "text-sm border-l-2",
                    isBlockedAction(e.action)
                      ? "border-severity-error bg-severity-error/[0.03]"
                      : "border-transparent",
                  )}
                >
                  <button
